...
if __name__ == "__main__":
    ...
    board = []
    for x in range(nVezir):
        board.append([" 0 "]*nVezir)

    for i in range(nVezir):
        board[nVezir-antsOut[i]][i] = " 1 "
    
    def printBoard(board):
        for row in board:
            print("".join(row))
            
    for i,item in enumerate(antsOut):
        antsOut[i] = item - 1
        
    end=time.time()
    show('Solution with ACO', antsOut, end-start)
    printBoard(board)
